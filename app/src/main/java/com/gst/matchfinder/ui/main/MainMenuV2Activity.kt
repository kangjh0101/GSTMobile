...
class MainMenuV2Activity : AppCompatActivity(), CoroutineScope by MainScope(), LifecycleObserver {
...
//Log.e("onCreate", "created")
        ...
        withdraw_page.setOnClickListener {
            val block_user_job = launch {
                val builder = AlertDialog.Builder(this@MainMenuV2Activity)
    ...
    private suspend fun withdraw_user(my_id: String){
        ...
        if(result == Constants.WITHDRAW_USER_SUCCESS){
            ...
            handler.post(kotlinx.coroutines.Runnable {
                Toast.makeText(this@MainMenuV2Activity, "탈퇴되었습니다.", Toast.LENGTH_LONG).show()
            })
            ...
            val handler = Handler(Looper.getMainLooper())
            handler.post(kotlinx.coroutines.Runnable {
                Toast.makeText(this@MainMenuV2Activity, "탈퇴하지 못했습니다.\n다시 시도해 주세요", Toast.LENGTH_LONG).show()
            })
